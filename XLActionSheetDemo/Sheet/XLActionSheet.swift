...
class XLActionSheetConfiguration: NSObject {
    /// 背景（半透颜色或模糊）动画进出（渐现）
    ...
    /// 标题高度
    @objc public var headerHeight: CGFloat = 50
    /// 点击了选项，自动隐藏
    @objc public var toucheCellAutoHide: Bool = true
}


class XLActionSheet: UIView {
    
    /// 配置
    @objc public var config: XLActionSheetConfiguration = XLActionSheetConfiguration()
    
    /// 选中回调
    @objc public var didSelectCell: ((_ cell: XLActionSheetCell,_ index: Int) -> Void)?
    /// 取消回调
    ...
    /// 对每个cell执行一次配置block
    @objc public func applyConfigForEachCell(_ config:((_ cell: XLActionSheetCell,_ index: Int) -> Void)) {
    ...
    /// 所有的Cells
    @objc public var cells: [XLActionSheetCell] = []
    
    /// 取消的那行cell
    @objc public var cancelCell: XLActionSheetCell = {
        let cell = XLActionSheetCell()
        cell.title = "取消"
        return cell
    }()
    ...
    @objc public var headerLab: UILabel = {
        ...
        lab.textColor = XLActionSheetConfiguration().headerTitleColor
    ...
    @objc init(headerTitle: String? = nil,
               actionTitles:[String],
               configuration: XLActionSheetConfiguration? = nil,
               cellCommonConfiguration: XLActionSheetCellConfiguration? = nil,
               selectCallBack: ((_ cell: XLActionSheetCell,_ index: Int) -> Void)? = nil,
    ...
    var backgroundView: UIView = {
        ...
        view.backgroundColor = XLActionSheetConfiguration().backgroudColor
        return view
    }()
    
    var cellView: UIScrollView = {
        let scrv = UIScrollView()
        scrv.backgroundColor = XLActionSheetConfiguration().cellClearanceColor
